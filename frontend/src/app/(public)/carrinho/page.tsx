...
import { montarLinkWhatsApp, montarMensagemPedido } from '@/lib/whatsapp';
...
export default function CarrinhoPage() {
  ...
  if (!hidratado) {
    return <div className="container py-16 text-center text-sm text-gray-400">Carregando carrinho...</div>;
  }
  ...
  if (itens.length === 0) {
    return (
      <div className="container flex flex-col items-center gap-4 py-24 text-center">
        ...
        <p className="max-w-sm text-gray-500">Navegue pelo catálogo e adicione os produtos que quiser incluir no seu pedido.</p>
  ...
  const linkWhatsApp = siteConfig.whatsappNumber
    ? montarLinkWhatsApp(siteConfig.whatsappNumber, montarMensagemPedido(itens))
    : null;
  ...
  return (
    <div className="container py-8">
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-3">
        ...
        <aside className="h-fit rounded-2xl border border-gray-100 bg-gray-50 p-5">
          ...
          <p className="mt-3 text-center text-xs text-gray-400">
            O pagamento e a negociação final acontecem diretamente com a vendedora pelo WhatsApp.
          </p>
        </aside>
      </div>
    </div>
